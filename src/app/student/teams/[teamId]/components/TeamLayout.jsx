...
export const TeamInfo = ({ title, description, meta, icon, className = '' }) => (
  <div className={`space-y-4 ${className}`} data-oid="1ak_.:5">
    <h1
      className="flex items-center gap-3 text-3xl font-bold text-gray-800 dark:text-white md:text-4xl"
      data-oid="pjzkbkn"
    >
      {icon && (
        <span className="text-2xl md:text-3xl" data-oid="-u:ap6e">
          {icon}
        </span>
      )}
      {title}
    ...
    {meta && (
      <div
        className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400 md:text-base"
        data-oid="1z1j92f"
      >
        {meta}
      </div>
...
export const HeaderActions = ({ children, className = '' }) => (
  <div className={`flex flex-wrap gap-3 md:gap-4 ${className}`} data-oid="ovx2hdk">
    {children}
  </div>
);
