...
export function DashboardCard({ title, icon, content, actionButton, stats }) {
  ...
  return (
    <div
      id="dashboard-card"
      className={`dashboard-card relative overflow-hidden rounded-xl bg-white p-4 shadow-md transition-all duration-300 md:p-6 ${
      ...
      {stats && (
        ...
        >
          {Object.entries(stats).map(([key, value]) => (
            ...
            >
              <span className="dashboard-card-stat-label mb-1 text-sm text-gray-500">
                {key}
              </span>
