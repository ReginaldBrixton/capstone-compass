...
const CalendarInput = ({
  ...
  const renderCalendar = () => {
    ...
    return (
      <div
        className="calendar-component w-[260px] rounded-lg border border-gray-200 bg-white p-3 shadow-xl transition-all duration-200 ease-in-out sm:w-[280px] sm:p-4 md:w-[300px] md:p-5"
        data-oid="-gxn4xn"
      >
        {title && (
          <div
            className="mb-3 text-center text-sm font-bold text-gray-800 sm:mb-4 sm:text-base md:mb-5 md:text-lg"
            data-oid="s9m4ytr"
          >
            {title}
          </div>
        )}
        <div className="mb-3 flex items-center justify-between sm:mb-4 md:mb-5" data-oid="46fg85q">
          <button
          ...
          <div className="flex items-center gap-2" data-oid="99qdelt">
            <button
        ...
        {showMonthYearSelect && (
          <div className="mb-3 grid grid-cols-2 gap-2 rounded-lg bg-gray-50 p-2" data-oid="adev5-r">
            <select
              value={currentMonth}
            ...
            >
              {months.map((month, index) => (
                <option key={month} value={index} data-oid="3nuqfyq">
                  {month}
                </option>
              ))}
            </select>
            ...
            >
              {years.map((year) => (
                <option key={year} value={year} data-oid="dx0xtb4">
                  {year}
                </option>
              ))}
            </select>
        ...
        <div className="mb-1 grid grid-cols-7 gap-1 sm:mb-2 sm:gap-1.5" data-oid="y1c1jnv">
          {daysOfWeek.map((day) => (
            <div
              key={day}
          ...
          {blanks.map((_, index) => (
            <div key={`blank-${index}`} className="text-center" data-oid="7r662af" />
          ))}
          {days.map((day) => {
            ...
            return (
              <button
                ...
                disabled={isDisabled}
                className={`rounded-full p-1 text-center text-xs font-medium transition-all duration-200 ease-in-out sm:p-1.5 sm:text-sm ${isSelected ? 'bg-blue-600 text-white shadow-sm hover:bg-blue-700' : isInRange ? 'bg-blue-100 text-blue-800 hover:bg-blue-200' : isDisabled ? 'cursor-not-allowed bg-gray-100 opacity-40' : 'text-gray-700 hover:bg-gray-100'} ${isToday && !isSelected ? 'ring-1 ring-blue-400' : ''} `}
                data-oid="3r9g6af"
              >
                {day}
              </button>
        ...
        {buttons && (
          <div
            className="mt-3 flex justify-between border-t border-gray-200 pt-2 sm:mt-4 sm:pt-3"
            data-oid="q7x2lmw"
          >
            <button
              onClick={handleToday}
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className={`absolute z-50 ${orientation === 'top' ? 'bottom-full mb-2' : orientation === 'left' ? 'right-full mr-2' : orientation === 'right' ? 'left-full ml-2' : 'top-full mt-2'}`}
          data-oid="._5pfkk"
        >
          {renderCalendar()}
        </div>
